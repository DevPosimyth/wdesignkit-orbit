...
if ( ! class_exists( 'Wdkit_Api_Call' ) ) {
	...
	class Wdkit_Api_Call {
		...
		public function wdkit_api_call() {
			...
			switch ( $type ) {
				...
				case 'wkit_check_job_status':
					$data = apply_filters( 'wp_Wdkit_Ai_Widget_Builder_Ajax', 'wkit_check_job_status' );
					break;
				case 'wkit_upload_design_image':
					$data = apply_filters( 'wp_Wdkit_Ai_Widget_Builder_Ajax', 'wkit_upload_design_image' );
					break;
				case 'wkit_check_upload_status':
					$data = apply_filters( 'wp_Wdkit_Ai_Widget_Builder_Ajax', 'wkit_check_upload_status' );
					break;
				case 'wkit_hitl_resume':
					$data = apply_filters( 'wp_Wdkit_Ai_Widget_Builder_Ajax', 'wkit_hitl_resume' );
					break;
				case 'wkit_hitl_cancel':
					$data = apply_filters( 'wp_Wdkit_Ai_Widget_Builder_Ajax', 'wkit_hitl_cancel' );
					break;
				case 'wkit_resume_checkpoint':
					$data = apply_filters( 'wp_Wdkit_Ai_Widget_Builder_Ajax', 'wkit_resume_checkpoint' );
					break;
				case 'wdkit_get_workspace_data':
					$data = $this->wdkit_get_workspace_data();
					break;
				case 'wkit_ai_check_token_status':
					$data = $this->wdkit_ai_check_token_status();
					break;
			}
		...
		/**
		 * Check token status (AI widget credits) – v2 endpoint.
		 * POST api/v2/wp/ai/widget/credits with token in body. Used by AI chatbox.
		 *
		 * @since 2.2.x
		 * @return array Decoded API response for wdkit_success_msg; or exits with wp_send_json_error on failure.
		 */
		public function wdkit_ai_check_token_status() {
			$token = isset( $_POST['token'] ) ? sanitize_text_field( wp_unslash( $_POST['token'] ) ) : '';
			if ( empty( $token ) ) {
				wp_send_json_error( array( 'message' => __( 'Token is required.', 'wdesignkit' ) ) );
			}

			$url  = $this->wdkit_api_v2 . 'ai/widget/credits';
			...
			if ( is_wp_error( $response ) ) {
				wp_send_json_error( array(
					'message' => __( 'Check token status failed: ', 'wdesignkit' ) . $response->get_error_message(),
				) );
			}
			...
			if ( 200 !== (int) $status_code ) {
				wp_send_json_error( array(
					'message' => isset( $data['message'] ) ? $data['message'] : __( 'Token status check failed.', 'wdesignkit' ),
					'code'    => $status_code,
					'data'    => $data,
				) );
			}
