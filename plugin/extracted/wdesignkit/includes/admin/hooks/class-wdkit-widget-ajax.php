		...
		public function wkit_widget_preview() {
			...
			if ( 'elementor' === $widget_type ) {
				...
				if ( class_exists( '\Elementor\Plugin' ) ) {
					...
					/**
					 * Pre-generate Elementor CSS during AJAX preview. Disabled by default: templates often
					 * reference kit globals (colors/typography) that are missing on this site, which triggers
					 * Elementor Error_404 during generate_css and can hit max_execution_time. The editor
					 * regenerates CSS when the preview URL loads.
					 *
					 * @param bool $generate Whether to run generate_css in this request.
					 * @param int  $page_id  Preview page ID.
					 */
					if ( apply_filters( 'wdkit_preview_elementor_generate_css', false, $page_id ) ) {
